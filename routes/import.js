...
router.get("/" , checkAccount ,async function (req,res){
    ...
    res.render("import/index" , {suppliers:suppliers, classes:classes , imports:imports});
});

router.post("/getItem" , checkAccount , async function (req , res) {
    const items = await Item.find({status : true , categoryId : req.body.categoryId
    });
    console.log(items);
...
router.post("/addItem" , checkAccount , async function(req , res){
    try{
        ...
        if(req.body.arrivalDate)
            imports.arrivalDate = req.body.arrivalDate;
        ...
        for(var i = 0; i < data.list.length; i++){
            let updated = await Item.findByIdAndUpdate(data.list[i].itemId, {
